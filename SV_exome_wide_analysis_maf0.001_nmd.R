...
cases <- read.delim("Rare_StructuralVariants_pkd_exonwide_275x_participants.txt", sep="\t")
controls <- read.delim("Rare_StructuralVariants_maf0.001rdcontrol_exonwide_25963x_participants.txt",sep="\t")
cases <- na.omit(cases)
...
panel <- fread("/re_gecip/renal/oalavijeh/projects/pkd_sv/analysis/exomewide_nmd/gencode_v29.bed", header=F)
...
write.table(df, paste(group,"_nmd_maf<0.001_filtered_exome_wide_SV_summary_stats.txt", sep=""), col.names=T, row.names=F, sep="\t", quote=F)
